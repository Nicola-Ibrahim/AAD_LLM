...
if ! curl -s http://localhost:$PORT/v1/models &>/dev/null; then
    ...
    echo "Please start the server first: bash server/scripts/02_serve_model.sh"
    exit 1
fi

# 2. Setup .env file if it doesn't exist
if [ ! -f "$PROJECT_ROOT/server/.env" ]; then
    echo "  [INFO] .env not found. Copying server/.env.server.example to server/.env..."
    cp "$PROJECT_ROOT/server/.env.server.example" "$PROJECT_ROOT/server/.env"
else
    # Make sure LLM_PROVIDER is configured for local server
    if grep -q "LLM_PROVIDER=gemini" "$PROJECT_ROOT/server/.env"; then
        echo "WARNING: Your .env is set to 'gemini'. To run locally on the server,"
        echo "please update your .env to use 'LLM_PROVIDER=lmstudio' as shown in server/.env.server.example."
        exit 1
    fi
fi
