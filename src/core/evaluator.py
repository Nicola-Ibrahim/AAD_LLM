    ...
    def __call__(self, solution: Solution, explogger: Any | None = None) -> Solution:
        ...
        try:
            ...
            if type(e).__name__ == 'FunctionTimedOut':
                ...
                metadata = {
                    "problem_id": self.problem.problem_id, "dim": self.problem.dim,
                    "noise_std": self.noise_std, "instance_id": self.problem.instance_id,
                    "true_optimum": self.problem.true_optimum,
                    "algorithm_returned_fitness": float('inf'),
                    "final_error": float('inf'),
                    "algorithm_name": solution.name, "timed_out": True
                }
            else:
                fitness_score = float('-inf')
                feedback = (
                ...
                metadata = {
                    ...
                    "algorithm_returned_fitness": float('inf'),
                    "final_error": float('inf'),
                    "algorithm_name": solution.name, "timed_out": False
                }
