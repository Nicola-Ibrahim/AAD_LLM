...
\cite{Burke2013}. The characteristics of the optimization problem therefore
matter when choosing or designing an optimization method.
...
structures \cite{Hansen2010}. The suite is integrated into experimental
frameworks such as \texttt{IOHexperimenter} \cite{deNobel2024} and COCO. The
known optima of the benchmark functions also make it possible to measure the
progress of an optimizer relative to a known reference solution.
...
2 & $f_6 - f_9$ & Low/Moderate Conditioning & Unimodal landscapes featuring smooth, single-valley search trajectories. \\
...
In electric-vehicle battery management, for example, parameter identification
can involve estimating equivalent-circuit parameters from measured sensor data
\cite{Plett2004,Xiong2018}. Thermal effects, electrical interference, and
measurement limitations can affect these observations. A similar situation
occurs in finite-element model updating, where vibration measurements are used
to identify or update structural models and are subject to measurement
uncertainty \cite{Mottershead1993,Marwala2010}. These examples illustrate why
an optimization algorithm may not always receive exactly the same objective
value for a given candidate solution.
...
These distributions lead to different types of uncertainty. Uniform noise
limits the disturbance to a fixed range, whereas Cauchy noise can produce
occasional large deviations because of its heavy tails. Gaussian noise gives
a continuous model in which the variance can be controlled through $\sigma$.
The choice of noise model therefore affects both the frequency and magnitude
of deviations between observed and underlying fitness.
...
The consequences depend on the optimization method. Direct search methods can
suffer from premature step-size reduction when objective differences become
smaller than the magnitude of the evaluation noise \cite{Arnold2003}. On
noisy sphere functions, for example, pattern search, Nelder--Mead, and
implicit-filtering approaches can interpret random fluctuations as
information about the local search direction. As the search radius decreases,
the actual objective differences can become comparable to the noise
magnitude. The resulting decisions may reduce the search radius even though
the current solution is still far from the optimum.
...
their measured fitness \cite{Hansen2001}. The generation mean can be written
as
...
\cite{Hansen2009Noisy,Sarafian2024}. Noise can therefore influence not only
which candidate is selected, but also the search distribution used in later
generations.
...
over programs. This program-space search can produce different combinations
of operations and algorithmic structures rather than choosing only among
...
AEL \cite{AEL2024} & Code Blocks & Combinatorial & Evolutionary Crossover & Deterministic Only \\
...
(ELA) features and uses them to guide algorithm generation
\cite{AutoEP2026}. This gives the model information about the characteristics
of the optimization problem before a candidate algorithm is generated.
...
instruction. Their use as variation mechanisms for continuous optimization
algorithm synthesis under stochastic fitness, however, has not been
established.
...
Table~\ref{tab:llm_aad_comparison} primarily evaluate generated algorithms
under deterministic conditions. Consequently, the effect of stochastic
fitness on the evolutionary selection process used by these systems remains
less explored.
...
Evolutionary Algorithms repeatedly perform initialization, variation,
fitness evaluation, and selection. An initial population can be represented
as
\begin{equation}
...
This property is particularly relevant when the individuals in the
...
The generation context can be represented as
...
The history helps prevent the generation process from repeatedly returning
identical candidates. The incumbent context provides the LLM with the
current algorithm and its observed performance. The mutation directive can
...
by the evaluation range. The mean AOCC across benchmark instances can be
used as the scalar fitness for candidate replacement in the original
framework \cite{LLaMEA2024}.
...
&
\text{if }
f(\mathcal{A}_{\text{child}}^{(g)})
\geq
f(\mathcal{A}_{\text{parent}}^{(g)})
\\
...
defined. The important point here is that one candidate is retained as the
parent, and this candidate becomes the starting point for the next
generation.

LLaMEA also uses execution feedback. If generated code raises an unhandled
runtime exception, such as a singular matrix inversion or division by zero,
execution terminates and the candidate receives a failed fitness value. The
traceback is retained in the incumbent context so that the LLM can use the
error information when generating the next candidate. Performance feedback
and execution failures can therefore both influence later program
modifications.

The dependence on a single parent creates a particular vulnerability under
stochastic evaluation. Consider a parent algorithm $\mathcal{A}_p$ and a
child algorithm $\mathcal{A}_c$. Their observed performance can be written
as
\begin{equation}
\widehat{F}(\mathcal{A}) =
...
$\epsilon_{\mathcal{A}}$ is an evaluation error.

A child that is genuinely worse than its parent can therefore receive a
favorable observation and be selected. Once selected, this child becomes the
basis for subsequent generations. This situation can be described as the
\emph{Ghost Parent} failure mode.

Under a favorable noise realization, a poorly performing child may appear to
achieve an unusually strong fitness score. Strict elitist selection can then
replace the actual parent with this statistically favored candidate. Since
subsequent LLM generations use the selected candidate as their incumbent,
future candidates are generated from an inferior algorithm. If later
evaluations do not reproduce the same favorable noise realization, the
selected parent can become difficult to replace even though its underlying
performance is worse.

The effect is therefore not limited to a single incorrect evaluation. A
noisy selection decision changes the state of the evolutionary search itself.
In a program-space search such as LLaMEA, evaluation noise can consequently
influence not only which algorithm is considered successful, but also which
algorithmic structure becomes the basis for subsequent generations.

This leads to an important distinction between evaluating an already generated
algorithm under noise and performing the entire algorithm-synthesis process
under noisy feedback. The first setting measures the robustness of an
algorithm after it has been generated. The second also tests whether the
evolutionary search process can reliably identify and retain useful
algorithmic structures when its fitness signal is stochastic.
...
LLM-based AAD frameworks reviewed in this chapter primarily evaluate generated
algorithms under deterministic conditions. The comparison in
Table~\ref{tab:llm_aad_comparison} shows that existing approaches use
different forms of performance, structural, reflective, or problem-specific
information, but stochastic fitness is not explicitly treated as a source of
uncertainty in the evolutionary selection process.
...
same conditions. It is also not yet clear whether the feedback-driven
generation process can produce algorithmic structures that compensate for
noisy evaluations.
